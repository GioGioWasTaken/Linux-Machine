/*This Is the second part of mmn12. What follows below is the Box3D class.
  ...
  or a base coordinate of type Point3D, paired with a 3 ordered tuple of the type 
  (height, width, length), as to represent a Box in a 3d coordinate space.
 * @author Iyar Segev
 * @version 1.0
  */
...
public class Box3D{
  ...
  public Box3D(Point3D base, int height, int width, int length){
   ...
   // Note for the grader, this condition can be achieved more concisely using the ternary operator ?
      // but to prioritize readability of code:
   if(height <= 0){
       this._height=1;
   }
...
// Returns a string representation of this Box3D object.
 ...
 public String toString(){
   return "The base point is "+ this.getBase().toString() + ", length = "+this._length+ ", width = " + this._width +", height = " + this._height;
 } 

  // determines if the boxes are equal
  public boolean equals(Box3D other){
    if(this._height == other._height && this._width == other._width && this._length == other._length){
      Point3D base = this.getBase();
      Point3D other_base = other.getBase();
      if(base.getX() ==other_base.getX() && base.getY() == other_base.getY() && base.getZ()== other_base.getZ()){
        return true;
      }
      return false;
    }
  ...
  public Point3D getUpRightBackPoint(){
      ...
      return new Point3D(base.getX() - this._length,base.getY() + this._width,base.getZ() + this._height);
  }

  // gets the center of the box and represents it as a Point3D coordinate
  public Point3D getCenter(){
      Point3D base = this.Point3D_base;
      return new Point3D(base.getX() - (double) (this._length) /2,base.getY() + (double) (this._width) /2,base.getZ() + (double) (this._height) /2);
      // I will not cast the box parameters to a double type, since it is not demanded.
  }
  ...
  public int getSurfaceArea(){
      ...
      return this._height*this._width*2 + this._length*this._width*2 +this._length*this._height*2;
  }
  ...
  public boolean contains(Box3D other){
      // a box B may be contained within box A, if and only if, box B's width length and height, are smaller than A's.
  ...
  public boolean isAbove(Box3D other){
      Point3D base = this.getBase();
      Point3D other_base = other.getBase();
      // for a box to be completely above another, we must also account for box length:
